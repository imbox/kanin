...
Topology.prototype.configure = function (cb) {
  var {exchanges, queues, bindings, replyQueue} = this
  ...
  async.series(
    [
      next => {
        amqp.connect(utils.amqpUrl(connectionOpts), (err, conn) => {
          connection = conn
          next(err)
        })
      },
      ...
      next => {
        setupReplyQueue(channel, replyQueue, onReply, next)
      }
    ],
    err => cb(err, {connection, channel})
  )
}
